...
export function buildTransferNoteContent(
  userName: string,
  items: CartItem[]
): string {
  ...
  return `${trimmed} - ${itemParts.join(", ")}`;
}

/** @deprecated Use buildTransferNoteContent — label is shown separately in UI */
export function buildTransferNote(userName: string, items: CartItem[]): string {
  return buildTransferNoteContent(userName, items);
}
